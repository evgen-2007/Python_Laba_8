...
# --- Дані Студента 3 Житник ---
# Цей скрипт доповнює файл task_exchange.txt
# відповіддю Студента 3 та новим питанням для Студента 4.

# Ім'я файлу, з яким ми всі працюємо (має бути в тій самій папці)
FILE_NAME = "task_exchange.txt"
STUDENT_LASTNAME = "Житник"

# Ваша відповідь на питання Студента 2:
# "Що таке словник (dict) у Python? Опишіть його структуру, 
# та наведіть приклади використання."
ANSWER_TEXT = (
    "Відповідь:\n"
...
# Ваше питання для наступного (четвертого) студента
QUESTION_TEXT = (
...
def append_to_file(filename, lastname, answer, question):
    """
    Додає дані (прізвище, відповідь, нове питання) у кінець файлу.
    Використовує режим 'a' (append).
    
    (Коментарі додано Студентом 3: Житник)
    """
    print(f"Спроба доповнити файл: {filename} (Студент: {lastname})")
    try:
        # 'a' - (append) - додає текст у кінець файлу, не стираючи старий вміст.
        # 'encoding='utf-8'' - обов'язково для коректної роботи з кирилицею.
        with open(filename, 'a', encoding='utf-8') as f:
            
            # Записуємо блок інформації третього студента
            f.write("--- Студент 3 ---\n")
            f.write(f"Прізвище: {lastname}\n")
            f.write(f"{answer}\n\n")  # Додаємо \n\n для відступу
            f.write("Питання:\n")
        ...
        # Ця помилка виникне, якщо файл task_exchange.txt не знайдено
        # у тій самій папці, де запущено скрипт.
        print(f"ПОМИЛКА: файл '{filename}' не знайдено. "
              "Переконайтеся, що ви завантажили його з репозиторію.")
    
    except PermissionError:
        # Виникає, якщо файл "тільки для читання" або немає прав на запис.
        print(f"ПОМИЛКА: Недостатньо прав для запису у файл '{filename}'.")

    except IOError as e:
        # Загальна помилка вводу/виводу (напр. якщо диск переповнений)
        print(f"ПОМИЛKA вводу/виводу: {e}")
...
# --- Запуск функції ---
# Цей блок коду виконається, коли Студент 3 запустить цей файл
if __name__ == "__main__":
    append_to_file(FILE_NAME, STUDENT_LASTNAME, ANSWER_TEXT, QUESTION_TEXT)
